...
private:
	...
	public:
		EqThread() :
				Thread(), mq(new Mutex), _q(new std::queue<struct EqJob<E>*>), alive(false)
		{
		}
			...
			try
			{
				while (alive)
				{
					synchronized(*mq)
					{
						...
						while (alive && !_q->empty() && (j = _q->front()) != NULL)
						{
							_q->pop();
							dispatch(j->listners, j->event);
						}
...
public:
	 ...
	 */
	EventQueue(void) :
			_listeners(new std::set<EventListener<T>*>), mx(new Mutex), runner(new EqThread<T>())
	{
		runner->Start(this);
	}
	/*!
...
public:
	...
	static bool Register(Factory<P, K>* factory)
	{
		...
		DLOG_LOG(" new size = " << _x_instances_->size());DLOG_LOGL(" )");
		DLOG_OUTF();
		return !contains;
	}
	/*!
...
template<class P, class K>
class Factory<P,K>::XFact:Factory<P, K>
{
...
public:
	 ...
	 */
	Attribute(const std::string& name) :__name(name){}
	/*!
	 * \brief Copy constructor of Attribute class.
	 * @param other copy origin
	 */
	Attribute(const Attribute<V>&other) :__name(other.__name){}
	virtual ~Attribute(){}
	...
	bool operator <(const util::Attribute<V> &other)const
	  ...
	  friend std::ostream& operator <<(std::ostream& o_s, const Attribute<V> & item)
	{
		o_s << " [ Attribute<V="<<typeid(V).name()<<"> = Type= "<<typeid(item).name() <<"; Name= "<< item.getName()<<" ]" ;
		return o_s;
...
public:
	 ...
	 */
	virtual void Insert(const Attribute<V>&k, const V& v)   throw (std::logic_error);
...
AttributeSet<V>::AttributeSet(const AttributeSet<V>&other) :
		__read_only(other.__read_only), __values(std::map<std::string, V>((other.__values))), __att_keys(
...
template<typename V>
const V& AttributeSet<V>::Get(const Attribute<V>&k) const throw (std::out_of_range)
{
...
template<typename V>
void AttributeSet<V>::Insert(const Attribute<V>&k, const V& v) throw (std::logic_error)
{
	DLOG_INF();
	if (__read_only){
		DLOG_LOGL("read only set!!");
		DLOG_OUTF();
 ...
 void AttributeSet<V>::__rm_value(const Attribute<V>&k)
{
	try
	{
		...
		// taken from Item 32 of Scott Myers' Effective STL
 		__att_keys.erase(std::remove(__att_keys.begin(), __att_keys.end(), k), __att_keys.end());
	} catch (...)
	{
	}
}
